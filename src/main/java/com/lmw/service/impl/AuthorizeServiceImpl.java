...
public class AuthorizeServiceImpl implements AuthorizeService {
    ...
    public String validateAndRegister(Account account, String code, String sessionId) {
        //构建 Redis 键
        String key = "email:" + sessionId + ":" + account.getEmail() + ":false";
        //判断是否存在该键值对
        if (Boolean.TRUE.equals(template.hasKey(key))){
            //通过键获取值
            ...
            //不为空则判断前后端验证码是否一致
                ...
                if (mapper.findAccountByUsernameOrEmail(account.getUsername())!=null){
    ...
    public boolean resetPassword(String password, String email) {
        return mapper.resetPasswordByEmail(encoder.encode(password),email)>0;
    }
